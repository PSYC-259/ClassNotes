...
"bonus- don't worry about it if you're new"
sample(1:100, 5) > 50 && sample(1:30, 5)< 20 # efficiency stops as soon as one is false
"4,3,5,2,1" #possible sample from the first one
"10,5,6,3,2" #possible sample from the second one
#prints False b/c it stops as soon as one condition isn't true 
```
...
```{r}
participant<- 1:7
iceCream<- cbind.data.frame(participant, myVector)
...
```{r}
tvShow<-c("shameless", "parks and rec", "westworld", "breaking bad")
ratings<- c(10,9,10,10)
```

3. Make this variable into a data frame.
```{r}
tvRatings<- cbind.data.frame(tvShow, ratings)
```
...
gearFour<-subset(mtcars, mtcars$gear==4)

subset(x=mtcars, subset= mtcars$gear==4)
```
